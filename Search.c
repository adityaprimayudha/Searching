...
int main(){
	mhs orang[100],temp;
	char yes_no,value_search[11];
	int i=0,j=0,k,pilihan,index,no,order,l,index_search,pilih_search,flag;
	clock_t mulai,end;
	double waktu;
	...
	if(pilihan==1){
		...
		if(i==0){
			...
			scanf(" %s", &value_search);
			printf("\nPilih metode search :\n1. Sequential Search\t2. Binary Search\nJawaban = \n");
			scanf("%d", &pilih_search);
			system("cls");
			if(pilih_search==1){
				for(i=0;i<j;i++){
					if(strcmp(value_search,orang[i].NPM)==0){
						flag=1;
						index_search=i;
					}
				...
				if(flag==1){
					printf("\n\nData NPM %s ditemukan, pada mahasiswa ke-%d\n", value_search,index_search+1);
				...
				while (m<=n && flag!=1){
					o=(m+n)/2;
					if(strcmp(value_search,orang[o].NPM)==0){
						flag=1;
						index_search=o;
					}
					else if(strcmp(value_search,orang[o].NPM)>0){
						m=o+1;
					}
					else{
						n=o-1;
				...
				if(flag==1){
					printf("\nData dengan NPM %s, ditemukan pada mahasiswa ke-%d!!", value_search,index_search+1);
					printf("\nBerikut datanya :\n");
					printf("\nNama :%s\nAlamat :%s\nNPM :%s\nTempat Lahir :%s\nTanggal Lahir :%s\nNomor Handphone :%s\nJenis Kelamin :%s\n\n",orang[index_search].nama,
							orang[index_search].alamat, orang[index_search].NPM, orang[index_search].tempat, orang[index_search].tanggal, orang[index_search].no_hp,
							orang[index_search].gender);
